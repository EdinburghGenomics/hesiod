...
# in mode main, give a run prefix to search eg. 2022032

mode_main(){
    runs=("$TOP_LEVEL_DIR"/"$pattern"*)

    for arun in "${runs[@]}" ; do

        [ -e "$arun" ] || { echo "No such dir $arun" >&2 ; break ; }

        echo "Processing $arun" >&2
        ...
        echo "Found $nn fast5.gz files in $arun" >&2
        sleep 5
...
mode_f5(){
    ...
    sum2=$(zcat "${f5}.gz" | md5sum - | awk '{print $1}')
    if [ "$sum1" != "$sum2" ] ; then
        echo "md5sum mismatch in ${f5}.gz"
        exit 1
    fi
    ...
    mv -vt "$TRASH/$f5_reldir" "${f5}.gz" "$arun/md5sums/$f5_rel.gz.md5"
}

if [ "$mode" = main ] ; then

    pattern="$2" # eg. 2022032
    mode_main | parallel -j 20
